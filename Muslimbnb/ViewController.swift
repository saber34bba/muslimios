...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        Muslimbnb.webView.setNeedsLayout()
    }
    
    func initWebView() {
        Muslimbnb.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(Muslimbnb.webView);
        Muslimbnb.webView.uiDelegate = self;
        Muslimbnb.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
        
        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = Muslimbnb.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = Muslimbnb.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        self.splashBkgView.frame = CGRect(x: splashBkgFrame.minX, y: statusBarHeight, width: splashBkgFrame.width, height: splashBkgFrame.height)
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !Muslimbnb.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        ...
        Muslimbnb.webView.load(URLRequest(url: launchUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            Muslimbnb.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                Muslimbnb.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(Muslimbnb.webView.estimatedProgress);
    ...
    deinit {
        Muslimbnb.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: Muslimbnb.webView)
        }
